...
def convert_timestamp_to_datetime64(series, timezone_offset=8, timestamp_unit='s', precision='s'):
    """
    Convert a series of timestamps to datetime64 with specified timezone and precision.

    Parameters:
    - series (pd.Series): Series of timestamps.
    - timezone_offset (int): Timezone offset from UTC (e.g., 8 for UTC+8).
    - timestamp_unit (str): Unit of the input timestamps ('s' for seconds, 'ms' for milliseconds).
...
def convert_str_to_datetime64(series, format, timezone_offset=8, precision='s'):
    """
    ...
    - precision (str): Precision for the datetime ('s' for seconds, 'T' for minutes, 'H' for hours).
    - strftime_format (str): String format for output datetime objects. 
